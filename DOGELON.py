...
- 10 posts/day:
  ...
  - 2 healthy crypto updates (CoinMarketCap top-10, 2 random coins each cycle) — text-only
...
- Telethon listens for raid posts and auto-smashes 👊 inline button, logging feedback
...
def post_timeline(text, image_path=None):
    try:
        ...
        print("❌ Error posting timeline:", e)
        return None, None
...
def get_latest_tweet_for_handle(handle):
    try:
        ...
        print(f"❌ Error fetching latest tweet for @{handle}: {e}")
    return None, None
...
def fetch_cmc_quote(symbol):
    try:
        ...
        print(f"❌ CMC fetch error for {symbol}: {e}")
        return None
...
async def daily_cycle():
    ...
    if not influencers:
        print(f"⚠️ {INFLUENCERS_FILE} is empty — influencer replies will be skipped.")
    elon_texts = load_lines(ELON_TEXTS)
    if not elon_texts:
        print(f"⚠️ {ELON_TEXTS} is empty — please add ELON lines.")

    while True:
        print("\n🚀 Starting new daily cycle:", datetime.now().isoformat())
        ...
        for idx, (action, delay) in enumerate(zip(actions, delays)):
            ...
            print(f"\n⏱ Action [{idx+1}/{TOTAL_PER_DAY}] -> {action} (scheduled after {delay}s)")
            ...
            if action == "elon_timeline":
                ...
                if tid:
                    ...
                    print(f"✅ Posted ELON timeline: {url}")
                else:
                    print("❌ Failed ELON timeline post")

            elif action == "healthy_timeline":
                # pick corresponding healthy coin from the preselected list
                if not healthy_coins:
                    print("⚠️ No healthy coins selected; skipping.")
                    continue
                ...
                if tid:
                    ...
                    print(f"✅ Posted healthy ({coin_symbol}): {url}")
                else:
                    print(f"❌ Failed healthy post for {coin_symbol}")

            elif action == "influencer_reply":
                if not influencers:
                    print("⚠️ No influencers configured — skipping reply.")
                    continue
                ...
                if not chosen:
                    print("⚠️ Could not find influencer tweet to reply to (skipping).")
                    continue

                # reply templates (human-like, include $ELON)
                reply_templates = [
                    "Nice point — $ELON vibes here 🔥",
                    "Agree — $ELON energy. Interesting take!",
                    "$ELON — that's worth watching 👀",
                ...
                if rtid:
                    ...
                    print(f"✅ Replied to @{chosen}'s tweet {chosen_url} -> {rurl}")
                else:
                    print(f"❌ Failed to reply to @{chosen} ({chosen_tid})")
        ...
        if elon_links_today:
            ...
            try:
                ...
                print("📩 Sent ELON links to Telegram group.")
            except Exception as e:
                print("❌ Failed to send ELON links to Telegram:", e)
        else:
            print("⚠️ No ELON links to send this cycle.")
        ...
        print(f"⏳ Cycle done. Sleeping {sleep_seconds/3600:.2f} hours until next cycle...")
        await asyncio.sleep(sleep_seconds)

# ===================== MAIN =====================
async def main():
    print("🚀 Starting bot (Telegram login required if first run)...")
    await client.start()
    print("✅ Telegram client started. Listening for raids and ready to post.")
